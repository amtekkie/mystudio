...
JSEOF;
    ...
    if ($subCols == 1) {
        $css .= <<<CSS
/* FMZ Forum Display Extras — Subforum list layout */
.fmz-subforum-cols { display: flex; flex-direction: column; margin-top: 4px; }
.fmz-subforum-cols .fmz-sf-item { display: flex; align-items: center; gap: 4px; padding: 2px 0; }
CSS;
        ...
        while (($pos = strpos($contents, $needle, $offset)) !== false) {
            ...
            $inner = preg_replace(
                '/(?<!<\/div>)(<a\b[^>]*>.*?<\/a>)(?!<\/div>)/s',
                '<div class="fmz-sf-item">$1</div>',
                $inner
            );
            $replacement = '<div class="forumbit_subforums smalltext fmz-subforum-cols">' . $inner . '</div>';
