<?php
/**
 * FMZ Editor Extras — Mini Plugin Init
 *
 * Provides:
...
$GLOBALS['fmz_ee_options'] = isset($fmz_plugin_options) ? $fmz_plugin_options : array();

if (!defined('IN_ADMINCP')) {
    $plugins->add_hook('global_end', 'fmz_ee_inject_config');
    $plugins->add_hook('xmlhttp',    'fmz_ee_xmlhttp');
}
 ...
 */
function fmz_ee_inject_config()
{
    global $headerinclude;

    $opts = $GLOBALS['fmz_ee_options'];
...
function fmz_ee_xmlhttp()
{
    ...
    if ($action == 'editorextras_proxy') {
        ...
        // Download the image
        $image_data = fmz_ee_fetch_url_raw($image_url);
        ...
        $attachment = fmz_ee_save_as_attachment($file, $mybb, $db);
 ...
 */
function fmz_ee_fetch_url_raw($url)
{
    if (function_exists('curl_init')) {
        $ch = curl_init();
        curl_setopt($ch, CURLOPT_URL, $url);
 ...
 */
function fmz_ee_save_as_attachment($file, $mybb, $db)
{
