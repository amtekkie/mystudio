/**
 * MyStudio Editor — Monaco-based theme file editor
 *
...
(function () {
    ...
    function init() {
        var cfgEl = document.getElementById('msEditorConfig');
        if (!cfgEl) return;
        ...
        var treeEl = document.getElementById('ms-file-tree');
        if (treeEl) {
            treeEl.addEventListener('contextmenu', function (e) {
                if (e.target === treeEl || e.target.closest('.ms-file-tree') === treeEl && !e.target.closest('.ms-tree-item')) {
                    e.preventDefault();
    ...
    function setupMonaco() {
        ...
        if (typeof require === 'undefined' || typeof require.config === 'undefined') {
            console.error('MyStudio Editor: Monaco loader not found');
            return;
        }
        ...
        require(['vs/editor/editor.main'], function () {
            ...
            monaco.editor.defineTheme('ms-light', {
                base: 'vs',
                inherit: true,
                rules: [],
                colors: {
            ...
            state.editor = monaco.editor.create(document.getElementById('ms-monaco'), {
                theme: 'ms-light',
            ...
            var placeholder = document.querySelector('.ms-monaco-placeholder');
    ...
    function loadEmmet() {
        try {
            ...
            console.warn('MyStudio Editor: Emmet could not be loaded', e);
        }
    }
    ...
    function bindEvents() {
        // File search filter
        var search = document.getElementById('ms-file-search');
        if (search) {
        ...
        // Sidebar buttons
        var btnNewFile = document.getElementById('ms-btn-newfile');
        if (btnNewFile) {
        ...
        var btnNewFolder = document.getElementById('ms-btn-newfolder');
        if (btnNewFolder) {
        ...
        // Save & Sync button
        var btnSaveSync = document.getElementById('ms-btn-savesync');
        if (btnSaveSync) {
        ...
        var btnCollapseAll = document.getElementById('ms-btn-collapse-all');
        if (btnCollapseAll) {
        ...
        var btnCollapse = document.getElementById('ms-btn-collapse');
        if (btnCollapse) {
    ...
    function setupResizeHandle() {
        var handle  = document.getElementById('ms-resize-handle');
        var sidebar = document.getElementById('ms-sidebar');
    ...
    function loadThemeTree(slug) {
        ...
        var treeEl = document.getElementById('ms-file-tree');
        treeEl.innerHTML = '<div class="ms-loading">Loading…</div>';
        ...
        Promise.all([
            ...
            if (data.error) {
                treeEl.innerHTML = '<div class="ms-error">' + escapeHtml(data.error) + '</div>';
                return;
            }
            state.fileTree = data;
            ...
            treeEl.innerHTML = '<div class="ms-error">Failed: ' + escapeHtml(err.message) + '</div>';
        });
    }
    ...
    function renderChildren(children, container, parentPath) {
        children.forEach(function (child) {
            ...
            if (child.type === 'directory') {
                el.className = 'ms-tree-dir';
                el.innerHTML =
                    '<div class="ms-tree-item ms-tree-folder" data-path="' + escapeAttr(childPath) + '" data-type="directory">' +
                    '  <span class="ms-tree-arrow"><i class="bi bi-chevron-right"></i></span>' +
                    '  <span class="ms-tree-icon">' + ICON_MAP.directory + '</span>' +
                    '  <span class="ms-tree-name">' + escapeHtml(child.name) + '</span>' +
                    '  <span class="ms-tree-folder-dirty" style="display:none">M</span>' +
                    '</div>' +
                    '<div class="ms-tree-children" style="display:none"></div>';

                var header = el.querySelector('.ms-tree-item');
                var childrenBox = el.querySelector('.ms-tree-children');
                var rendered = false;
                ...
                (function (ch, cBox, cPath) {
                    header.addEventListener('click', function () {
                        var arrow = this.querySelector('.ms-tree-arrow');
                        var icon  = this.querySelector('.ms-tree-icon');
                ...
                el.className = 'ms-tree-file';
                var icon = ICON_MAP[child.ext] || ICON_MAP._default;
                el.innerHTML =
                    '<div class="ms-tree-item ms-tree-file-item" ' +
                    'data-path="' + escapeAttr(childPath) + '" data-type="file" data-ext="' + (child.ext || '') + '">' +
                    '  <span class="ms-tree-icon">' + icon + '</span>' +
                    '  <span class="ms-tree-name">' + escapeHtml(child.name) + '</span>' +
                    '  <span class="ms-tree-dirty" style="display:none">\u25CF</span>' +
                    '</div>';

                (function (path) {
                    var item = el.querySelector('.ms-tree-item');
    ...
    function filterFileTree(query) {
        ...
        matches.forEach(function (filePath) {
            ...
            el.className = 'ms-tree-file';
            el.innerHTML =
                '<div class="ms-tree-item ms-tree-file-item" data-path="' + escapeAttr(filePath) + '" data-type="file">' +
                '  <span class="ms-tree-icon">' + icon + '</span>' +
                '  <span class="ms-tree-name">' + escapeHtml(name) +
                '    <span style="color:#666;font-size:11px;margin-left:6px">' + escapeHtml(dir) + '</span>' +
                '  </span>' +
                '</div>';

            (function (p) {
                el.querySelector('.ms-tree-item').addEventListener('click', function () {
                    openFile(p);
                });
            })(filePath);
    ...
    function collapseAllFolders() {
        ...
        var childrenBoxes = treeEl.querySelectorAll('.ms-tree-children');
        for (var i = 0; i < childrenBoxes.length; i++) {
            childrenBoxes[i].style.display = 'none';
        }

        // Reset all folder arrows and icons
        var folderItems = treeEl.querySelectorAll('.ms-tree-folder');
        for (var j = 0; j < folderItems.length; j++) {
            var arrow = folderItems[j].querySelector('.ms-tree-arrow');
            var icon  = folderItems[j].querySelector('.ms-tree-icon');
    ...
    function switchToTab(path) {
        ...
        if (tab && state.editor) {
            ...
            var ph = document.querySelector('.ms-monaco-placeholder');
    ...
    function saveCurrentFile() {
        ...
        var statusEl = document.getElementById('ms-status-sync');
        statusEl.textContent = 'Saving…';
        statusEl.className   = 'ms-status-saving';
        ...
        .then(function (data) {
            if (data.success) {
                ...
                statusEl.className   = 'ms-status-saved';
                showNotification('Saved & synced to database', 'success');
                renderTabs();
                updateDirtyIndicators();

                setTimeout(function () {
                    if (statusEl.className === 'ms-status-saved') {
    ...
    function renderTabs() {
        var tabBar = document.getElementById('ms-tab-bar');
        if (!tabBar) return;
        tabBar.innerHTML = '';

        state.openTabs.forEach(function (tab) {
            var el = document.createElement('div');
            el.className = 'ms-tab' + (tab.path === state.activeTab ? ' active' : '');
            ...
            el.innerHTML =
                ...
                '<span class="ms-tab-name">' + escapeHtml(tab.name) + '</span>' +
                (tab.isDirty ? '<span class="ms-tab-dirty">\u25CF</span>' : '') +
                '<span class="ms-tab-close" title="Close">\u00D7</span>';

            (function (p) {
                el.addEventListener('click', function () { switchToTab(p); });
                el.querySelector('.ms-tab-close').addEventListener('click', function (e) { closeTab(p, e); });
            })(tab.path);
    ...
    function updateDirtyIndicators() {
        ...
        var fileItems = document.querySelectorAll('.ms-tree-file-item');
        for (var i = 0; i < fileItems.length; i++) {
            var path = fileItems[i].getAttribute('data-path');
            var dot  = fileItems[i].querySelector('.ms-tree-dirty');
            if (dot) {
        ...
        var folderItems = document.querySelectorAll('.ms-tree-folder');
        for (var j = 0; j < folderItems.length; j++) {
            var fPath = folderItems[j].getAttribute('data-path');
            var badge = folderItems[j].querySelector('.ms-tree-folder-dirty');
            if (badge) {
    ...
    function updateStatusBar() {
        var posEl  = document.getElementById('ms-status-pos');
        var langEl = document.getElementById('ms-status-lang');
    ...
    function showNotification(message, type) {
        var container = document.getElementById('ms-notifications');
        if (!container) return;

        var el = document.createElement('div');
        el.className = 'ms-notify ms-notify-' + (type || 'info');
    ...
    function setTreeItemActive(path) {
        var all = document.querySelectorAll('.ms-tree-item.active');
        for (var i = 0; i < all.length; i++) all[i].classList.remove('active');

        var item = document.querySelector('.ms-tree-item[data-path="' + CSS.escape(path) + '"]');
    ...
    function showContextMenu(x, y, path, type) {
        ...
        items.forEach(function (item) {
            if (item.sep) {
                ...
                sep.className = 'ms-context-menu-sep';
                menu.appendChild(sep);
                return;
            }

            var el = document.createElement('div');
            el.className = 'ms-context-menu-item' + (item.danger ? ' danger' : '');
            el.innerHTML = '<span class="ms-context-menu-icon">' + item.icon + '</span>' + escapeHtml(item.label);
    ...
    function closeContextMenu() {
        var existing = document.getElementById('ms-context-menu');
