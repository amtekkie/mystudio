/**
 * MyStudio — Page Editor (HTML + Monaco)
 *
 * Simple HTML editor for MyStudio pages using Monaco Editor.
...
{$footer}`;
    ...
    function savePage() {
        ...
        fetch('index.php?module=mystudio-pages_api&action=pages_api', {
            method: 'POST',
            body: formData,
        })
        .then(r => r.json())
        .then(data => {
            if (data.error) {
                ...
                if (!window.location.search.includes('pid=')) {
                    history.replaceState(null, '', 'index.php?module=mystudio-pages_edit&action=pages_edit&pid=' + pid);
                }
            }
        })
    ...
    function initGroups() {
        ...
        if (!container) return;
        const groups = window.MS_PAGE_USERGROUPS || [];
        groups.forEach(g => {
    ...
    function initVariables() {
        $$('.pb-var-item').forEach(el => {
            el.addEventListener('click', () => {
                ...
                editor.executeEdits('ms-insert-var', [{
                    range: selection,
                    text: varText,
    ...
    function openPreview() {
        const slug = $('#pb-slug').value.trim();
        if (slug && pid > 0) {
            const bburl = window.MS_PAGE_BBURL || '';
    ...
    function init() {
        ...
        const pageData = window.MS_PAGE_DATA || {};
